        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (none == null) {
                    ...
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo2065transformType = none.mo2065transformType(typeCheckerState, it.next());
                        if ((typeSystemContext.isClassType(rigidTypeMarkerMo2065transformType) && !typeSystemContext.isMarkedNullable(rigidTypeMarkerMo2065transformType)) || typeSystemContext.isDefinitelyNotNullType(rigidTypeMarkerMo2065transformType)) {
                            typeCheckerState.clear();
                            return true;
                        }
                        supertypesDeque.add(rigidTypeMarkerMo2065transformType);
                    }
                }
            }
        }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(rigidTypeMarkerPop)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(rigidTypeMarkerPop)).iterator();
                    while (it.hasNext()) {
                        RigidTypeMarker rigidTypeMarkerMo2065transformType = supertypesPolicy.mo2065transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, rigidTypeMarkerMo2065transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(rigidTypeMarkerMo2065transformType);
