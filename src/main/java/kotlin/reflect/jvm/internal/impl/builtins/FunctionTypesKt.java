...
public final class FunctionTypesKt {
    public static final FunctionTypeKind getFunctionTypeKind(KotlinType kotlinType) {
        ...
        if (classifierDescriptorMo2061getDeclarationDescriptor != null) {
            return getFunctionTypeKind(classifierDescriptorMo2061getDeclarationDescriptor);
        }
        return null;
    }
    ...
    public static final boolean isBuiltinFunctionalType(KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        ClassifierDescriptor classifierDescriptorMo2061getDeclarationDescriptor = kotlinType.getConstructor().mo2061getDeclarationDescriptor();
        return classifierDescriptorMo2061getDeclarationDescriptor != null && isBuiltinFunctionalClassDescriptor(classifierDescriptorMo2061getDeclarationDescriptor);
    }
    ...
    private static final boolean isTypeAnnotatedWithExtensionFunctionType(KotlinType kotlinType) {
        return kotlinType.getAnnotations().mo2056findAnnotation(StandardNames.FqNames.extensionFunctionType) != null;
    }
    ...
    public static final int contextFunctionTypeParamsCount(KotlinType kotlinType) {
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        AnnotationDescriptor annotationDescriptorMo2056findAnnotation = kotlinType.getAnnotations().mo2056findAnnotation(StandardNames.FqNames.contextFunctionTypeParams);
        if (annotationDescriptorMo2056findAnnotation == null) {
            return 0;
        }
        ConstantValue constantValue = (ConstantValue) MapsKt.getValue(annotationDescriptorMo2056findAnnotation.getAllValueArguments(), StandardNames.CONTEXT_FUNCTION_TYPE_PARAMETER_COUNT_NAME);
    ...
    public static final Name extractParameterNameFromFunctionTypeArgument(KotlinType kotlinType) {
        String value;
        Intrinsics.checkNotNullParameter(kotlinType, "<this>");
        AnnotationDescriptor annotationDescriptorMo2056findAnnotation = kotlinType.getAnnotations().mo2056findAnnotation(StandardNames.FqNames.parameterName);
        if (annotationDescriptorMo2056findAnnotation == null) {
            return null;
        }
