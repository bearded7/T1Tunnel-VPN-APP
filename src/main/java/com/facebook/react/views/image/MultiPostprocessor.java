...
public final class MultiPostprocessor implements Postprocessor {
    ...
    public CloseableReference<Bitmap> process(Bitmap sourceBitmap, PlatformBitmapFactory bitmapFactory) {
        ...
        try {
            CloseableReference<Bitmap> closeableReferenceMo217clone = null;
            for (Postprocessor postprocessor : this.postprocessors) {
                if (closeableReferenceMo217clone == null || (bitmap = closeableReferenceMo217clone.get()) == null) {
                    bitmap = sourceBitmap;
                }
                closeableReferenceProcess = postprocessor.process(bitmap, bitmapFactory);
                CloseableReference.closeSafely(closeableReferenceMo217clone);
                closeableReferenceMo217clone = closeableReferenceProcess.mo217clone();
            }
            ...
            CloseableReference<Bitmap> closeableReferenceMo217clone2 = closeableReferenceProcess.mo217clone();
            Intrinsics.checkNotNullExpressionValue(closeableReferenceMo217clone2, "clone(...)");
            CloseableReference.closeSafely(closeableReferenceProcess);
            return closeableReferenceMo217clone2;
        } catch (Throwable th) {
