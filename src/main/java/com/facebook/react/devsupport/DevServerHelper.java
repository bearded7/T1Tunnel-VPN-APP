...
public class DevServerHelper {
    ...
    public DevServerHelper(DeveloperSettings settings, Context applicationContext, PackagerConnectionSettings packagerConnectionSettings) {
        ...
        OkHttpClient okHttpClientBuild = new OkHttpClient.Builder().connectTimeout(CoroutineLiveDataKt.DEFAULT_TIMEOUT, TimeUnit.MILLISECONDS).readTimeout(0L, TimeUnit.MILLISECONDS).writeTimeout(0L, TimeUnit.MILLISECONDS).build();
    ...
    private final String createBundleURL(String mainModuleID, BundleType type, String host, boolean modulesOnly, boolean runModule) {
        ...
        StringCompanionObject stringCompanionObject = StringCompanionObject.INSTANCE;
        String str = String.format(Locale.US, "http://%s/%s.%s?platform=android&dev=%s&lazy=%s&minify=%s&app=%s&modulesOnly=%s&runModule=%s", Arrays.copyOf(new Object[]{host, mainModuleID, type.getTypeID(), Boolean.valueOf(devMode), Boolean.valueOf(devMode), Boolean.valueOf(getJSMinifyMode()), this.packageName, modulesOnly ? "true" : Constants.CASEFIRST_FALSE, runModule ? "true" : Constants.CASEFIRST_FALSE}, 9));
        Intrinsics.checkNotNullExpressionValue(str, "format(...)");
    ...
    public final void openDebugger(final ReactContext context, final String errorMessage) {
        ...
        Intrinsics.checkNotNullExpressionValue(str, "format(...)");
        this.client.newCall(new Request.Builder().url(str).method(ShareTarget.METHOD_POST, RequestBody.INSTANCE.create((MediaType) null, "")).build()).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.openDebugger.1
