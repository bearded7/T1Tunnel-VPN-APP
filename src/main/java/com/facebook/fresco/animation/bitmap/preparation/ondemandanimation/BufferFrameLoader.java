...
public final class BufferFrameLoader implements FrameLoader {
    ...
    public FrameResult getFrame(int frameNumber, int width, int height) {
        ...
        if (bufferFrame != null) {
            ...
            return new FrameResult(bufferFrame.getBitmapRef().mo217clone(), FrameResult.FrameType.SUCCESS);
        }
        loadNextFrames(width, height);
    ...
    private final FrameResult findNearestToRender(int targetFrame) {
        ...
        if (animationBitmapFrameFindNearestFrame != null) {
            CloseableReference<Bitmap> closeableReferenceMo217clone = animationBitmapFrameFindNearestFrame.getBitmap().mo217clone();
            Intrinsics.checkNotNullExpressionValue(closeableReferenceMo217clone, "clone(...)");
            this.lastRenderedFrameNumber = animationBitmapFrameFindNearestFrame.getFrameNumber();
            return new FrameResult(closeableReferenceMo217clone, FrameResult.FrameType.NEAREST);
        }
        ...
        while (it.hasNext()) {
            ...
            if (this.bufferFramesHash.get(Integer.valueOf(iIntValue2)) == null) {
                ...
                if (closeableReferenceCloneOrNull == null) {
                    ...
                    closeableReferenceCloneOrNull = bufferFrame.getBitmapRef().mo217clone();
                }
