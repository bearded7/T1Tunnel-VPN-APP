...
public final class InetAddresses {
    ...
    private static byte[] textToNumericFormatV6(String str) {
        ...
        if (i2 >= 0) {
            int size2 = listSplitToList.size() - i2;
            i = size2 - 1;
            ...
            if (((String) Iterables.getLast(listSplitToList)).length() == 0 && (i = size2 - 2) != 0) {
                return null;
            }
        } else {
        ...
        while (i > 0) {
            byteBufferAllocate.putShort(parseHextet(listSplitToList.get(listSplitToList.size() - i)));
            i--;
        }
