...
public final class BroadcastChannelImpl<E> extends BufferedChannel<E> implements BroadcastChannel<E> {
        ...
        try {
            ...
            if (!isClosedForSend() || this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                if (this.lastConflatedElement != BroadcastChannelKt.NO_ELEMENT) {
                    subscriberConflated.mo2291trySendJP2dKIU(getValue());
                }
    ...
    public Object mo2291trySendJP2dKIU(E element) {
        ...
        try {
            ...
            if (!(list instanceof Collection) || !list.isEmpty()) {
                ...
                while (it.hasNext()) {
                    if (((BufferedChannel) it.next()).shouldSendSuspend$kotlinx_coroutines_core()) {
                        Object objM2320failurePtdJZtk = ChannelResult.INSTANCE.m2320failurePtdJZtk();
                        reentrantLock.unlock();
                        return objM2320failurePtdJZtk;
                    }
                }
            }
            if (this.capacity == -1) {
            ...
            while (it2.hasNext()) {
                ((BufferedChannel) it2.next()).mo2291trySendJP2dKIU(element);
            }
            Object objM2321successJP2dKIU = ChannelResult.INSTANCE.m2321successJP2dKIU(Unit.INSTANCE);
            reentrantLock.unlock();
            return objM2321successJP2dKIU;
        } catch (Throwable th) {
            reentrantLock.unlock();
            throw th;
        }
