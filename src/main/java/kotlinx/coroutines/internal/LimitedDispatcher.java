...
public final class LimitedDispatcher extends CoroutineDispatcher implements Delay {
    ...
    public void mo2358scheduleResumeAfterDelay(long timeMillis, CancellableContinuation<? super Unit> continuation) {
        this.$$delegate_0.mo2358scheduleResumeAfterDelay(timeMillis, continuation);
    }
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (i >= 16 && LimitedDispatcher.this.dispatcher.isDispatchNeeded(LimitedDispatcher.this)) {
                    LimitedDispatcher.this.dispatcher.mo2357dispatch(LimitedDispatcher.this, this);
                    return;
                }
            }
        }
    ...
    public void mo2357dispatch(CoroutineContext context, Runnable block) {
        Runnable runnableObtainTaskOrDeallocateWorker;
        this.queue.addLast(block);
        if (runningWorkers$volatile$FU.get(this) >= this.parallelism || !tryAllocateWorker() || (runnableObtainTaskOrDeallocateWorker = obtainTaskOrDeallocateWorker()) == null) {
            return;
        }
        this.dispatcher.mo2357dispatch(this, new Worker(runnableObtainTaskOrDeallocateWorker));
    }
