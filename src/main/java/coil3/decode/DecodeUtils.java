...
public final class DecodeUtils {
    ...
    public static final long m112computeDstSizesEdh43o(int srcWidth, int srcHeight, Size targetSize, Scale scale, Size maxSize) {
        ...
        if ((maxSize.getWidth() instanceof Dimension.Pixels) && !UtilsKt.isMinOrMax(srcWidth)) {
            srcWidth = RangesKt.coerceAtMost(srcWidth, ((Dimension.Pixels) maxSize.getWidth()).m161unboximpl());
        }
        if ((maxSize.getHeight() instanceof Dimension.Pixels) && !UtilsKt.isMinOrMax(srcHeight)) {
            srcHeight = RangesKt.coerceAtMost(srcHeight, ((Dimension.Pixels) maxSize.getHeight()).m161unboximpl());
        }
        return IntPair.m166constructorimpl(srcWidth, srcHeight);
    }

    private final int toPx(Dimension dimension, Scale scale) {
        if (dimension instanceof Dimension.Pixels) {
            return ((Dimension.Pixels) dimension).m161unboximpl();
        }
        int i = WhenMappings.$EnumSwitchMapping$0[scale.ordinal()];
        if (i == 1) {
