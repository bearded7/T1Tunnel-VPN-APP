...
public final class RoundedCornersTransformation extends Transformation {
    ...
    public Object transform(Bitmap bitmap, Size size, Continuation<? super Bitmap> continuation) {
        ...
        long jM162calculateOutputSizenmZnaPc = m162calculateOutputSizenmZnaPc(bitmap, size);
        int iM170getFirstimpl = IntPair.m170getFirstimpl(jM162calculateOutputSizenmZnaPc);
        int iM171getSecondimpl = IntPair.m171getSecondimpl(jM162calculateOutputSizenmZnaPc);
        Bitmap bitmapCreateBitmap = Bitmap.createBitmap(iM170getFirstimpl, iM171getSecondimpl, BitmapsKt.getSafeConfig(bitmap));
        Canvas canvas = new Canvas(bitmapCreateBitmap);
        canvas.drawColor(0, PorterDuff.Mode.CLEAR);
        Matrix matrix = new Matrix();
        float fComputeSizeMultiplier = (float) DecodeUtils.computeSizeMultiplier(bitmap.getWidth(), bitmap.getHeight(), iM170getFirstimpl, iM171getSecondimpl, Scale.FILL);
        float f = 2;
        matrix.setTranslate((iM170getFirstimpl - (bitmap.getWidth() * fComputeSizeMultiplier)) / f, (iM171getSecondimpl - (bitmap.getHeight() * fComputeSizeMultiplier)) / f);
    ...
    private final long m162calculateOutputSizenmZnaPc(Bitmap input, Size size) {
        if (SizeKt.isOriginal(size)) {
            return IntPair.m166constructorimpl(input.getWidth(), input.getHeight());
        }
        Dimension width = size.getWidth();
        Dimension height = size.getHeight();
        if ((width instanceof Dimension.Pixels) && (height instanceof Dimension.Pixels)) {
            return IntPair.m166constructorimpl(((Dimension.Pixels) width).m161unboximpl(), ((Dimension.Pixels) height).m161unboximpl());
        }
        int width2 = input.getWidth();
        int height2 = input.getHeight();
        Dimension width3 = size.getWidth();
        int iM161unboximpl = width3 instanceof Dimension.Pixels ? ((Dimension.Pixels) width3).m161unboximpl() : Integer.MIN_VALUE;
        Dimension height3 = size.getHeight();
        double dComputeSizeMultiplier = DecodeUtils.computeSizeMultiplier(width2, height2, iM161unboximpl, height3 instanceof Dimension.Pixels ? ((Dimension.Pixels) height3).m161unboximpl() : Integer.MIN_VALUE, Scale.FILL);
        return IntPair.m166constructorimpl(MathKt.roundToInt(((double) input.getWidth()) * dComputeSizeMultiplier), MathKt.roundToInt(dComputeSizeMultiplier * ((double) input.getHeight())));
    }
}
