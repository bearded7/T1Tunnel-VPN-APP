...
final class LimitedFileDescriptorHardwareBitmapService implements HardwareBitmapService {
    ...
    public boolean allowHardwareMainThread(Size size) {
        ...
        if ((width instanceof Dimension.Pixels ? ((Dimension.Pixels) width).m161unboximpl() : Integer.MAX_VALUE) <= 100) {
            return false;
        }
        Dimension height = size.getHeight();
        return (height instanceof Dimension.Pixels ? ((Dimension.Pixels) height).m161unboximpl() : Integer.MAX_VALUE) > 100;
    }
